...
public class OpenCLContext {
	...
	public String[] getOpenCLPrograms() throws IOException {
		...
		for (String content : resourceNamesContent) {
			...
			contentWithReplacements = contentWithReplacements.replaceAll("GLOBAL_AS const secp256k1_t \\*tmps",
					"const secp256k1_t \\*tmps");
	...
	// TODO der auszuführende Kernel
	private final static String KERNEL_NAME = "generateKeysKernel_grid";

	// TODO need to store in a 3-dimensional array (cube)
	private final static String NEW_KERNEL_NAME = "generateKeyAddressPairsKernel_cube";
	...
	public OpenCLGridResult createKeys(BigInteger privateKeyBase) {
		...
		OpenCLGridResult openCLGridResult = new OpenCLGridResult(privateKeyBase, producerOpenCL.getWorkSize(),
				dstByteBuffer);
	...
	public OpenCLCubeResults createKeyAddressPairs(BigInteger secretBase) {

		if (false) {
			// TODO was macht das?
			openClTask.setSrcPrivateKeyChunk(secretBase);
		}

		ByteBuffer dstByteBuffer = null;

		if (false) {
			dstByteBuffer = openClTask.executeKernel(kernel, commandQueue);
		}

		OpenCLCubeResults openCLGridResult = new OpenCLCubeResults(secretBase, producerOpenCL.getWorkSize(),
				dstByteBuffer);
		return openCLGridResult;
	}
}