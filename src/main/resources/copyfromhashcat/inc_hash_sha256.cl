...
// input buf needs to be in algorithm native byte order (md5 = LE, sha256 = BE,
// etc) input buf needs to be 64 byte aligned when using sha256_update()

DECLSPEC void sha256_transform(PRIVATE_AS const u32 *w0,
                               PRIVATE_AS const u32 *w1,
                               PRIVATE_AS const u32 *w2,
                               PRIVATE_AS const u32 *w3,
                               PRIVATE_AS u32 *digest) {
  u32 a = digest[0];
  u32 b = digest[1];
  u32 c = digest[2];
  u32 d = digest[3];
...
DECLSPEC void sha256_update_64(PRIVATE_AS sha256_ctx_t *ctx, PRIVATE_AS u32 *w0,
                               PRIVATE_AS u32 *w1, PRIVATE_AS u32 *w2,
                               PRIVATE_AS u32 *w3, const int len) {
  if (len == 0)
    return;
...
DECLSPEC void sha256_update_swap(PRIVATE_AS sha256_ctx_t *ctx,
                                 PRIVATE_AS const u32 *w, const int len) {
  u32 w0[4];
  u32 w1[4];
  u32 w2[4];
  u32 w3[4];
...
DECLSPEC void sha256_update_utf16le(PRIVATE_AS sha256_ctx_t *ctx,
                                    PRIVATE_AS const u32 *w, const int len) {
  if (hc_enc_scan(w, len)) {
    ...
    while (hc_enc_has_next(&hc_enc, len)) {
      ...
      const int enc_len =
          hc_enc_next(&hc_enc, w, len, 256, enc_buf, sizeof(enc_buf));
      ...
      sha256_update_64(ctx, enc_buf + 0, enc_buf + 4, enc_buf + 8, enc_buf + 12,
                       enc_len);
    }
...
DECLSPEC void sha256_update_utf16le_swap(PRIVATE_AS sha256_ctx_t *ctx,
                                         PRIVATE_AS const u32 *w,
                                         const int len) {
  if (hc_enc_scan(w, len)) {
    hc_enc_t hc_enc;
...
DECLSPEC void sha256_update_global(PRIVATE_AS sha256_ctx_t *ctx,
                                   GLOBAL_AS const u32 *w, const int len) {
  u32 w0[4];
  u32 w1[4];
  u32 w2[4];
  u32 w3[4];
...
DECLSPEC void sha256_update_global_swap(PRIVATE_AS sha256_ctx_t *ctx,
                                        GLOBAL_AS const u32 *w, const int len) {
  u32 w0[4];
  u32 w1[4];
  u32 w2[4];
...
DECLSPEC void sha256_update_global_utf16le(PRIVATE_AS sha256_ctx_t *ctx,
                                           GLOBAL_AS const u32 *w,
                                           const int len) {
  if (hc_enc_scan_global(w, len)) {
    ...
    while (hc_enc_has_next(&hc_enc, len)) {
      ...
      const int enc_len =
          hc_enc_next_global(&hc_enc, w, len, 256, enc_buf, sizeof(enc_buf));
...
DECLSPEC void sha256_update_global_utf16le_swap(PRIVATE_AS sha256_ctx_t *ctx,
                                                GLOBAL_AS const u32 *w,
                                                const int len) {
  if (hc_enc_scan_global(w, len)) {
    hc_enc_t hc_enc;
...
DECLSPEC void sha256_hmac_init_64(PRIVATE_AS sha256_hmac_ctx_t *ctx,
                                  PRIVATE_AS const u32 *w0,
                                  PRIVATE_AS const u32 *w1,
                                  PRIVATE_AS const u32 *w2,
                                  PRIVATE_AS const u32 *w3) {
  u32 a0[4];
...
DECLSPEC void sha256_hmac_init(PRIVATE_AS sha256_hmac_ctx_t *ctx,
                               PRIVATE_AS const u32 *w, const int len) {
  u32 w0[4];
  u32 w1[4];
  u32 w2[4];
...
DECLSPEC void sha256_hmac_init_swap(PRIVATE_AS sha256_hmac_ctx_t *ctx,
                                    PRIVATE_AS const u32 *w, const int len) {
  u32 w0[4];
  u32 w1[4];
  u32 w2[4];
...
DECLSPEC void sha256_hmac_init_global(PRIVATE_AS sha256_hmac_ctx_t *ctx,
                                      GLOBAL_AS const u32 *w, const int len) {
  u32 w0[4];
  u32 w1[4];
  u32 w2[4];
...
DECLSPEC void sha256_hmac_init_global_swap(PRIVATE_AS sha256_hmac_ctx_t *ctx,
                                           GLOBAL_AS const u32 *w,
                                           const int len) {
  u32 w0[4];
  u32 w1[4];
...
DECLSPEC void sha256_hmac_update_64(PRIVATE_AS sha256_hmac_ctx_t *ctx,
                                    PRIVATE_AS u32 *w0, PRIVATE_AS u32 *w1,
                                    PRIVATE_AS u32 *w2, PRIVATE_AS u32 *w3,
                                    const int len) {
  sha256_update_64(&ctx->ipad, w0, w1, w2, w3, len);
}

DECLSPEC void sha256_hmac_update(PRIVATE_AS sha256_hmac_ctx_t *ctx,
                                 PRIVATE_AS const u32 *w, const int len) {
  sha256_update(&ctx->ipad, w, len);
}

DECLSPEC void sha256_hmac_update_swap(PRIVATE_AS sha256_hmac_ctx_t *ctx,
                                      PRIVATE_AS const u32 *w, const int len) {
  sha256_update_swap(&ctx->ipad, w, len);
}

DECLSPEC void sha256_hmac_update_utf16le(PRIVATE_AS sha256_hmac_ctx_t *ctx,
                                         PRIVATE_AS const u32 *w,
                                         const int len) {
  sha256_update_utf16le(&ctx->ipad, w, len);
}

DECLSPEC void sha256_hmac_update_utf16le_swap(PRIVATE_AS sha256_hmac_ctx_t *ctx,
                                              PRIVATE_AS const u32 *w,
                                              const int len) {
  sha256_update_utf16le_swap(&ctx->ipad, w, len);
}

DECLSPEC void sha256_hmac_update_global(PRIVATE_AS sha256_hmac_ctx_t *ctx,
                                        GLOBAL_AS const u32 *w, const int len) {
  sha256_update_global(&ctx->ipad, w, len);
}

DECLSPEC void sha256_hmac_update_global_swap(PRIVATE_AS sha256_hmac_ctx_t *ctx,
                                             GLOBAL_AS const u32 *w,
                                             const int len) {
...
DECLSPEC void
sha256_hmac_update_global_utf16le(PRIVATE_AS sha256_hmac_ctx_t *ctx,
                                  GLOBAL_AS const u32 *w, const int len) {
  sha256_update_global_utf16le(&ctx->ipad, w, len);
}

DECLSPEC void
sha256_hmac_update_global_utf16le_swap(PRIVATE_AS sha256_hmac_ctx_t *ctx,
                                       GLOBAL_AS const u32 *w, const int len) {
...
// while input buf can be a vector datatype, the length of the different
// elements can not

DECLSPEC void sha256_transform_vector(PRIVATE_AS const u32x *w0,
                                      PRIVATE_AS const u32x *w1,
                                      PRIVATE_AS const u32x *w2,
                                      PRIVATE_AS const u32x *w3,
...
#define ROUND_STEP(i)                                                          \
  {                                                                            \
    SHA256_STEP(SHA256_F0o, SHA256_F1o, a, b, c, d, e, f, g, h, w0_t,          \
                k_sha256[i + 0]);                                              \
    SHA256_STEP(SHA256_F0o, SHA256_F1o, h, a, b, c, d, e, f, g, w1_t,          \
                k_sha256[i + 1]);                                              \
    SHA256_STEP(SHA256_F0o, SHA256_F1o, g, h, a, b, c, d, e, f, w2_t,          \
                k_sha256[i + 2]);                                              \
    SHA256_STEP(SHA256_F0o, SHA256_F1o, f, g, h, a, b, c, d, e, w3_t,          \
                k_sha256[i + 3]);                                              \
    SHA256_STEP(SHA256_F0o, SHA256_F1o, e, f, g, h, a, b, c, d, w4_t,          \
                k_sha256[i + 4]);                                              \
    SHA256_STEP(SHA256_F0o, SHA256_F1o, d, e, f, g, h, a, b, c, w5_t,          \
                k_sha256[i + 5]);                                              \
    SHA256_STEP(SHA256_F0o, SHA256_F1o, c, d, e, f, g, h, a, b, w6_t,          \
                k_sha256[i + 6]);                                              \
    SHA256_STEP(SHA256_F0o, SHA256_F1o, b, c, d, e, f, g, h, a, w7_t,          \
                k_sha256[i + 7]);                                              \
    SHA256_STEP(SHA256_F0o, SHA256_F1o, a, b, c, d, e, f, g, h, w8_t,          \
                k_sha256[i + 8]);                                              \
    SHA256_STEP(SHA256_F0o, SHA256_F1o, h, a, b, c, d, e, f, g, w9_t,          \
                k_sha256[i + 9]);                                              \
    SHA256_STEP(SHA256_F0o, SHA256_F1o, g, h, a, b, c, d, e, f, wa_t,          \
                k_sha256[i + 10]);                                             \
    SHA256_STEP(SHA256_F0o, SHA256_F1o, f, g, h, a, b, c, d, e, wb_t,          \
                k_sha256[i + 11]);                                             \
    SHA256_STEP(SHA256_F0o, SHA256_F1o, e, f, g, h, a, b, c, d, wc_t,          \
                k_sha256[i + 12]);                                             \
    SHA256_STEP(SHA256_F0o, SHA256_F1o, d, e, f, g, h, a, b, c, wd_t,          \
                k_sha256[i + 13]);                                             \
    SHA256_STEP(SHA256_F0o, SHA256_F1o, c, d, e, f, g, h, a, b, we_t,          \
                k_sha256[i + 14]);                                             \
    SHA256_STEP(SHA256_F0o, SHA256_F1o, b, c, d, e, f, g, h, a, wf_t,          \
                k_sha256[i + 15]);                                             \
  }
...
DECLSPEC void
sha256_init_vector_from_scalar(PRIVATE_AS sha256_ctx_vector_t *ctx,
                               PRIVATE_AS sha256_ctx_t *ctx0) {
  ctx->h[0] = ctx0->h[0];
  ctx->h[1] = ctx0->h[1];
  ctx->h[2] = ctx0->h[2];
  ctx->h[3] = ctx0->h[3];
...
DECLSPEC void sha256_update_vector_64(PRIVATE_AS sha256_ctx_vector_t *ctx,
                                      PRIVATE_AS u32x *w0, PRIVATE_AS u32x *w1,
                                      PRIVATE_AS u32x *w2, PRIVATE_AS u32x *w3,
                                      const int len) {
  if (len == 0)
    return;
...
DECLSPEC void sha256_update_vector(PRIVATE_AS sha256_ctx_vector_t *ctx,
                                   PRIVATE_AS const u32x *w, const int len) {
  u32x w0[4];
  u32x w1[4];
  u32x w2[4];
  u32x w3[4];
...
DECLSPEC void sha256_update_vector_swap(PRIVATE_AS sha256_ctx_vector_t *ctx,
                                        PRIVATE_AS const u32x *w,
                                        const int len) {
  u32x w0[4];
  u32x w1[4];
  u32x w2[4];
...
DECLSPEC void sha256_update_vector_utf16le(PRIVATE_AS sha256_ctx_vector_t *ctx,
                                           PRIVATE_AS const u32x *w,
                                           const int len) {
  u32x w0[4];
  u32x w1[4];
...
DECLSPEC void
sha256_update_vector_utf16le_swap(PRIVATE_AS sha256_ctx_vector_t *ctx,
                                  PRIVATE_AS const u32x *w, const int len) {
  u32x w0[4];
  u32x w1[4];
  u32x w2[4];
...
DECLSPEC void sha256_update_vector_utf16beN(PRIVATE_AS sha256_ctx_vector_t *ctx,
                                            PRIVATE_AS const u32x *w,
                                            const int len) {
  u32x w0[4];
  u32x w1[4];
...
DECLSPEC void
sha256_hmac_init_vector_64(PRIVATE_AS sha256_hmac_ctx_vector_t *ctx,
                           PRIVATE_AS const u32x *w0, PRIVATE_AS const u32x *w1,
                           PRIVATE_AS const u32x *w2,
                           PRIVATE_AS const u32x *w3) {
  u32x a0[4];
  u32x a1[4];
...
DECLSPEC void sha256_hmac_init_vector(PRIVATE_AS sha256_hmac_ctx_vector_t *ctx,
                                      PRIVATE_AS const u32x *w, const int len) {
  u32x w0[4];
  u32x w1[4];
  u32x w2[4];
...
DECLSPEC void
sha256_hmac_update_vector_64(PRIVATE_AS sha256_hmac_ctx_vector_t *ctx,
                             PRIVATE_AS u32x *w0, PRIVATE_AS u32x *w1,
                             PRIVATE_AS u32x *w2, PRIVATE_AS u32x *w3,
                             const int len) {
...
DECLSPEC void
sha256_hmac_update_vector(PRIVATE_AS sha256_hmac_ctx_vector_t *ctx,
                          PRIVATE_AS const u32x *w, const int len) {
  sha256_update_vector(&ctx->ipad, w, len);
}

DECLSPEC void
sha256_hmac_final_vector(PRIVATE_AS sha256_hmac_ctx_vector_t *ctx) {
