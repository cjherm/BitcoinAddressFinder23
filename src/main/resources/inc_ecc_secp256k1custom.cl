...
// decimal:
47440210799387980664936216788675555637818488436833759923669526136462528967388
// WiF
 ...
 * a size of 8.
 */
__kernel void generateKeysKernel_parse_public(__global u32 *r,
  ...
  // global to local
  // from global memory to local memory of context?
  // why?
  k_local[0] = k[0];
  k_local[1] = k[1];
  k_local[2] = k[2];
  ...
  // Point multiplication of r * k
  point_mul(r_local, k_local, &g_xy_local);

  // local to global
  // back from local memory of context? to global memory
  // why?
  r[0] = r_local[0];
  r[1] = r_local[1];
  r[2] = r_local[2];
...
/*
 * Generate a secp256k1_t struct for the public point. pre-computed points:
 * (x1,y1,-y1),(x3,y3,-y3),(x5,y5,-y5),(x7,y7,-y7).
 * @param r out: secp256k1_t structure, a pointer to an u32 array with a size of
 * 96 (SECP256K1_PRE_COMPUTED_XY_SIZE).
 */
...
__kernel void generateKeysKernel_transform_public(__global u32 *r,
                                                  __global const u32 *k) {
  u32 g_local[PUBLIC_KEY_LENGTH_WITHOUT_PARITY];
  u32 r_local[PUBLIC_KEY_LENGTH_WITH_PARITY];
  u32 k_local[PRIVATE_KEY_LENGTH];
